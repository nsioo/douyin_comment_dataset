...
def create_thread(name, url_cookies_path, save_to):

    # 改一下!!##############################
    # url_cookies_path = './re_run/dance_ch.xls'
    # tp = 'dance'
    # number = 1
    #url_cookies_path = './single_vedio/star/video_'+str(number)+'_url.xls'
    # save_to  = './res/'+tp+'_result_'+str(number)+'.txt'
    ########################################
...
if __name__ == "__main__":
    save_path = 'res'
    input_path = 're_run'
    input_name = ['star_5000_ch']
    p = Pool(int(cpu_count() * 0.75))
    for name in input_name:
        input_p = os.path.join(input_path, name+'.xls')
        save_p = os.path.join(save_path, name+'.txt')
        p.apply_async(create_thread, args=(name, input_p, save_p))
